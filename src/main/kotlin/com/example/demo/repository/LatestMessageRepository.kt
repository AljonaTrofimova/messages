...
interface LatestMessageRepository : JpaRepository<LatestMessageWithCount, Long> {
    @Query(value = "select top 1 (select count(*) from message) as total_saved_messages, * from message group by(id, text, created) ORDER BY CREATED DESC;", nativeQuery = true)
